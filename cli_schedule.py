...
def main():
    ...
    build_all_tasks(args)

def build_all_tasks(args: Optional[argparse.Namespace]):
    ...
    daily_tasks = price_scheduler_client.generate_daily_tasks(
        entity_id=args.entity_id if args and args.entity_id else None
    )
    ...
    monthly_tasks = price_scheduler_client.generate_monthly_tasks(
        entity_id=args.entity_id if args and args.entity_id else None,
        min_date=datetime.strptime(args.min_date, "%Y-%m-%dT%H:%M:%S") if args and args.min_date else None
    )
